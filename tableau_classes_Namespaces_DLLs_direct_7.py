...
decompile_root = r"D:\BUREAU-BUREAU-BUREAU-BUREAU-BUREAU\FORMATION E3D ADMIN\DLL decompilation\transposition_dll_all_2"
...
CSHARP_FAMILIES = {
    ...
    'float': 'Property', 'bool': 'Property', 'get': 'Property', 'set': 'Property', 'new': 'Constructor',
    'Hashtable': 'Property', 'DbElement': 'Property'
}

def detect_family(line):
    line = line.strip()

    # Étape 0 : ligne incomplète (ex: "private" seul)
    if re.fullmatch(r'\b(public|private|internal|protected)\b', line):
        return "INCOMPLETE"

    # Étape 1 : mots-clés standards connus
    ...
    # Étape 2 : détection des classes "sealed" ou "unsafe sealed"
    if re.search(r'\b(public|private|internal|protected)\s+(unsafe\s+)?sealed\s+class\b', line):
        return "Class"

    # Étape 3 : détection des membres "new static TYPE" → potentielle méthode ou propriété
    if re.search(r'\b(public|private|internal|protected)\s+new\s+static\s+\w+\s+\w+\s*\(', line):
        return "Method"
    elif re.search(r'\b(public|private|internal|protected)\s+new\s+static\s+\w+\s+\w+\s*[;{]', line):
        return "Property"

    # Étape 4 : détection générique d'une méthode (return type + nom + parenthèses)
    if re.search(r'\b(public|private|internal|protected)(\s+unsafe)?\s+\w+\s+\w+\s*\(', line):
        return "Method"

    # Étape 5 : détection générique d'une propriété (type + nom + ; ou {)
    if re.search(r'\b(public|private|internal|protected)(\s+unsafe)?\s+\w+\s+\w+\s*[;{]', line):
        return "Property"

    # Étape 6 : méthode AVEVA spécifique (nom de méthode directement après le modificateur)
...
def extract_name(line, family):
    try:
        if family == "Class":
            ...
            return re.findall(r'event\s+\w+\s+(\w+)', line)[0]
            ...
            match = re.findall(r'\b\w+\s+(\w+)\s*(?:\{|\(|;)', line)
